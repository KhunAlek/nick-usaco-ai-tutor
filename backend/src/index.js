...
async function getToday(env, cors) {
  const row = await env.DB.prepare(`
    ...
    WHERE d.learner_id='nick' ORDER BY d.learning_date DESC LIMIT 1
  `).first();
  if (!row) return json({ error: "No learning day is scheduled" }, 404, cors);
  return json({
    day_id: row.day_id,
...
async function submit(request, env, cors) {
  ...
  const day = await env.DB.prepare("SELECT * FROM learning_days WHERE learner_id='nick' ORDER BY learning_date DESC LIMIT 1").first();
  if (!day) return json({ error: "No learning day is scheduled" }, 409, cors);
  if (day.status !== "OPEN") return json({ error: "Today’s learning day is already closed" }, 409, cors);
  ...
  let generatedLessonId = null;
  if (generatedLesson) {
    generatedLessonId = crypto.randomUUID();
  }
  ...
  if (evaluation.outcome === "PASSED") {
    statements.push(env.DB.prepare("UPDATE learning_days SET status='CLOSED',outcome='PASSED',closed_at=? WHERE id=?").bind(now, day.id));
    ...
    statements.push(env.DB.prepare("UPDATE learning_days SET lesson_id=?,outcome='SMALL_CORRECTION' WHERE id=?").bind(generatedLessonId, day.id));
  } else {
  ...
  return json({
    review: {
      ...evaluation,
      generated_lesson: generatedLesson ? { id: generatedLessonId, ...generatedLesson } : null,
      day_status: evaluation.outcome === "SMALL_CORRECTION" ? "OPEN" : "CLOSED"
    }
  }, 200, cors);
}
...
async function getHistory(env, cors) {
  const rows = await env.DB.prepare(`
    SELECT d.learning_date,d.status,d.outcome,l.title,l.lesson_type
    FROM learning_days d JOIN lessons l ON l.id=d.lesson_id
    WHERE d.learner_id='nick' ORDER BY d.learning_date DESC
  `).all();
      ...
      generated_lesson: {
        anyOf: [
          { type: "null" },
          {
            type: "object",
            additionalProperties: false,
            required: ["title","instructions","required_evidence"],
            properties: {
...
async function readJson(request) {
  ...
  if (new TextEncoder().encode(text).length > MAX_BODY_BYTES) throw new Error("Submission too large");
  return JSON.parse(text);
}
